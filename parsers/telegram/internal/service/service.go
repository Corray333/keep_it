...
type external interface {
	GetTgPhoto(photos []tgbotapi.PhotoSize) ([]byte, error)
}
...
func (s *Service) ParseMessage(ctx context.Context, message *tgbotapi.Message) error {
	...
	if message.ForwardFromChat != nil {
		...
		note.CopiedAt = time.Unix(int64(message.ForwardDate), 0)
	...
	if message.Photo != nil {
		...
		coverKey := viper.GetString("s3.default_prefix") + "/" + uuid.New().String() + ".png"

		if err := s.fileManger.SaveFile(context.Background(), tgPhoto, coverKey); err != nil {
			slog.Error("Error saving file", "error", err)
			return err
		}

		note.Cover = viper.GetString("s3.base_url") + viper.GetString("s3.bucket") + "/" + coverKey
	...
	fmt.Println("Finsl:", finalNote.ContentDecoded)
